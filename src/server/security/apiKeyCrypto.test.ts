...
describe("apiKeyCrypto", () => {
  ...
  it("returns settings metadata without exposing encrypted values", () => {
    const encrypted = encryptGeminiApiKey("AIza-test-user-secret-key", TEST_SECRET, { userId: USER_ID });

    const metadata = toGeminiApiKeySettingsMetadata(encrypted);
    ...
    expect(toGeminiApiKeySettingsMetadata()).toEqual({ configured: false });
  });

  it("rejects secrets that do not decode to 32 bytes", () => {
    expect(() => decodeApiKeyEncryptionSecret(Buffer.from("too-short").toString("base64"))).toThrow("32 bytes");
  });
});
